...
namespace ContactCenterDA.Repositories.CC.TH
{
    public class ReservaRepository : IReservaRepository
    {
        ...
        {
            string sql = "INSERT INTO TH_RESERVA (FECHARESERVA,HORARIO,idEstadoReserva,idObra,idFuncion,idCliente,idUsuario,NombrePromocion,FechaCrea,userCrea,PrecioTotal,Asientos,NombreCliente,Empresa,CorreoCliente) " +
                            "values(@fechaReserva,@horario,@idEstadoReserva,@idObra,@idFuncion,@idCliente,@idUsuario,@nombrePromocion,@fechaCrea,@userCrea,@PrecioTotal,@Asientos,@NombreCliente,@Empresa,@CorreoCliente)";
            ...
            OleDbParameter pCorreoCliente = UtilDA.SetParameters("@CorreoCliente", OleDbType.VarChar, datos.Cliente.Correo);
            ...
            int id = UtilDA.ExecuteNonQueryGetId(cmd, CommandType.Text, sql, cnx, true, pFechaReserva, pHorario, pEstadoReserva, pIdObra, pIdFuncion, pIdCliente,
                pIdUsuario, pNombrePromo, pFechaCrea, pUserCrea, pTotal, pAsientos, pNombreCliente, pEmpresa, pCorreoCliente);
            ...
            if (datos.ListaDetalles.Where(tx => tx.NombrePromocion == "CORTESIA").ToList().Count > cantidadCortesiasRestante)
            {
                UtilDA.ExecuteRollback(cmd, cnx);
            ...
            using (var dtr = UtilDA.ExecuteReader(cmd, CommandType.Text, sql, cnx, pNombrePhone, pFecha,pFechaFin))
            {
                while (dtr.Read())
                {
                    reserva = new BusquedaReserva()
                    {
                        ...
                        NombreCliente = DataConvert.ToString(dtr["NombreCliente"]),
                        Empresa = DataConvert.ToString(dtr["Empresa"]),
                        CorreoCliente = DataConvert.ToString(dtr["CorreoCliente"]),
